...
session_start();
  ...
  while($row=mysqli_fetch_array($c)){
          ...
          $cid = $row["cid"];
            //$cname = $row["cname"];"books_details.php?bid='.$bid.'"
    $c_option .= '<a class="dropdown-item text-info english" href="choose_categories.php?cid='.$cid.'">'.$row['cname'].'</a>';
  }
  # for authors
  ...
  while($row=mysqli_fetch_array($a)){
          $aid = $row["aid"];
            //$cname = $row["cname"];
    $a_option .= '<a class="dropdown-item text-info english" href="choose_authors.php?aid='.$aid.'">'.$row['aname'].' </a>';
...
<br>
  ...
  <?php
    ...
    $sql_books = "select books.*,authors.aname from books,authors
              where books.aid = authors.aid  ORDER BY bid DESC";
